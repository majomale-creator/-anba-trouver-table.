...
const ALIASES=[
 {heard:["milo","miano"],targets:["gnianou","gnanou"]},
 {heard:["basket"],targets:["basque"]},
 {heard:["carly"],targets:["karuna"]},
 {heard:["moua"],targets:["moutapa"]},
 {heard:["bas morgan"],targets:["morgan"]}
];
function aliasTarget(t){
 t=norm(t);
 for(const a of ALIASES) if(a.heard.includes(t)){
   const p=PEOPLE.find(p=>a.targets.some(x=>p.nkey===norm(x)||p.key.includes(norm(x))));
   if(p)return p;
 }
 return null;
}
...
function choices(t,voice=false){
 c.innerHTML=""; if(!norm(t))return;
 const known=aliasTarget(t); if(voice&&known){show(known);return}
 let r=rank(t),a=r[0],b=r[1];
 if(voice&&a&&a.s>=.86&&(!b||a.s-b.s>=.10)){show(a.p);return}
 r.slice(0,3).filter(x=>x.s>.38).forEach(x=>{
   let b=document.createElement("button");b.className="cand";
   b.textContent=x.p.full+" — table "+x.p.table;b.onclick=()=>show(x.p);c.appendChild(b)
 });
}
...
if(SR){let rec=new SR();if ("SpeechRecognitionPhrase" in window && "phrases" in rec) {
  rec.phrases = [
    new SpeechRecognitionPhrase("Gnianou", 8),
    new SpeechRecognitionPhrase("Karuna", 8),
    new SpeechRecognitionPhrase("Moutapa", 8),
    new SpeechRecognitionPhrase("Basque", 8),
    new SpeechRecognitionPhrase("Morgan", 8)
  ];
}rec.lang="fr-FR";rec.interimResults=false;rec.maxAlternatives=10;mic.onclick=()=>{try{rec.start()}catch(e){}};rec.onstart=()=>{mic.classList.add("on");mic.textContent="● J'ÉCOUTE…";status.textContent="Prononcez le nom.";heard.textContent=""};rec.onend=()=>{mic.classList.remove("on");mic.textContent="🎤 DIRE LE NOM"};rec.onerror=e=>status.textContent="Erreur micro : "+e.error;rec.onresult=e=>{
 ...
 let all=[];alts.forEach(t=>rank(t).slice(0,4).forEach(x=>all.push({...x,text:t})));
 all.sort((a,b)=>b.s-a.s);
 if(all[0]){
   let next=all.find(x=>x.p!==all[0].p);
   if(all[0].s>=.86&&(!next||all[0].s-next.s>=.10))show(all[0].p);
