// ANBA app.js — correction phonétique ciblée v2 (17-09-2026)
let PEOPLE=[];
...
function score(t,p){
  t=norm(t).replace(/\b(monsieur|madame|mr|mme|mister|mrs|miss)\b/g,"").trim();
  if(!t)return 0;

  const words=t.split(" ").filter(Boolean);
  const rev=norm(p.nom+" "+p.prenom);
  let best=0;

  // 1) Une personne complète, dans l'ordre prénom+nom OU nom+prénom.
  // C'est le cas important pour "Nalini Mustapha" -> "Nalini Mouttapa"
  // et "Marie Christine Mathieu" -> "Marie Christine Matthieu".
  for(const k of [p.key,rev]){
    if(!k)continue;
    if(k===t) best=1;
    ...
    if(ps>=.68){
      // Si la phrase entière sonne presque pareil, elle doit peser beaucoup
      // plus qu'un simple prénom reconnu à l'intérieur de la phrase.
      const phScore = ps>=.90 ? .98 : Math.min(.94,.60+.34*ps);
      best=Math.max(best,phScore);
    }
  }

  // 2) Si l'utilisateur n'a prononcé QU'UN seul mot, on autorise
  // naturellement la recherche par nom ou par prénom seul.
  // En revanche, dans une phrase de plusieurs mots, "Nalini" seul ne doit
  // plus donner artificiellement 0,94 à toutes les Nalini.
  if(words.length===1){
    for(const k of [p.nkey,p.pkey]){
      if(!k)continue;
      if(k===t) best=Math.max(best,1);
      if(k.includes(t)||t.includes(k)) best=Math.max(best,.94);
      best=Math.max(best,sim(t,k));
      const ps=sim(phon(t),phon(k));
      if(ps>=.68) best=Math.max(best,Math.min(.90,.60+.34*ps));
    }
  }else{
    // Pour plusieurs mots, un prénom ou un nom complet peut encore aider,
    // mais seulement comme indice secondaire, jamais comme résultat quasi exact.
    for(const k of [p.nkey,p.pkey]){
      if(!k)continue;
      if(k===t) best=Math.max(best,.90);
      else if(t.includes(k) && k.length>=4) best=Math.max(best,.72);
    }
  }

  // Tous les mots réellement reconnus et présents dans la fiche restent un bon indice.
  let z=words.filter(x=>x.length>1);
  if(z.length&&z.every(x=>p.key.includes(x)||rev.includes(x))) best=Math.max(best,.97);
  return best;
}
